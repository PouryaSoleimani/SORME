...
const RegisterPage = ({ }) => {
  ...
  const schema = yup.object().shape({
    ...
    password: yup.string().required('Password is Required').min(10 , 'Password Must be Longer')
  }).required();

  //&REACT-HOOK-FORM
  const { register, handleSubmit, formState: { errors }, reset } = useForm<Inputs>({ resolver: yupResolver(schema) })
  const sendData = (data: {}) => { axios.post('http://localhost:3000/users', data).then(response => console.log(response)).catch(error => console.log(error)) }
  //*SUBMIT HANDLER
        ...
        <div id="form_container" className="w-[33rem] h-fit bg-[#F29AA7] rounded-3xl flex flex-col px-2 scale-75">
          ...
          <form suppressHydrationWarning onSubmit={handleSubmit(onSubmit)} className='flex flex-col p-6 space-y-6 border-b-4 border-dotted border-white px-4'>
            ...
            <span className='flex flex-col'>
              ...
              <input type="text" placeholder='Sourosh Rasouli' {...register("username")} className='px-5 py-5 z-0 border-4 placeholder:text-xl relative left-0 placeholder:text-white border-white bg-transparent rounded-lg w-full text-3xl text-white font-semibold' />
            ...
            <span className='flex flex-col'>
              ...
              <input type="text" placeholder='bndfjbf@gmailcom' {...register("email")} className='px-5 py-5 z-0 border-4 placeholder:text-xl relative left-0 placeholder:text-white border-white bg-transparent rounded-lg w-full text-3xl text-white font-semibold' />
              <FaUser className="text-white w-10 h-12 absolute translate-y-3 translate-x-[26.5rem]" />
